...
% FontAwesome5 requires LuaLaTeX or XeLaTeX - using text fallback
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, fit, backgrounds, calc, shadows, shapes.multipart, shapes.symbols}
...
% 1. INPUT LAYER
% =================================================================================
\node[io] (input_voice) {$\bigcirc$ Audio Stream};
\node[io, right=0.5cm of input_voice] (input_text) {$\boxed{\text{T}}$ Text Input};
...
\node[model, below=0.6cm of qwen, dashed, draw=gray, text=gray, fill=white, text width=3cm] (llama) {
    $\mathcal{L}$ \textbf{Explanation (VI)}\\
    LLaMA3-8B\\
...
\node[io, below=0.5cm of tts] (output) {$\triangleright$ Audio Response};
\node[io, right=0.5cm of output] (json) {$\langle\rangle$ JSON Stream};
